...
| 15 | May 30, 2026 | Medium | Seed / Auth | "Clear All Seed Data" didn't remove test-signup accounts (real users who signed up during testing). Root cause: `clearAll()` only deletes rows where `is_seed_data = true`; sign-up accounts have no such flag. Added separate `clearAllTestAccounts()` action + two-step UI in admin seed panel (preview → confirm). Gates on `NEXT_PUBLIC_ENABLE_DEMO_LOGIN === 'true'` so it's invisible in production. `src/lib/seed/clear-test-accounts.ts` (new), `src/actions/seed.ts`, `src/components/admin/seed-runner.tsx`. | ✅ FIXED |
| 16 | May 30, 2026 | High | Seed / Demo | Demo accounts `demo.admin@llv.dev` and `demo.client@llv.dev` referenced throughout the app but never seeded — logging in would fail. Added `seedDemoAccounts()` script: creates both auth users, demo client is fully onboarded (WI + AZ addresses, Seasonal Essentials subscription, 6 wardrobe items, 1 completed seasonal rotation order). Script is idempotent (skips if accounts exist). Added as "Demo Accounts" in `SEED_MANIFEST` and included in `seedAll()` Tier 1 parallel batch. `src/lib/seed/seed-demo-accounts.ts` (new), `src/lib/seed/seed-all.ts`, `src/lib/seed/manifest.ts`. | ✅ FIXED |
| 17 | May 30, 2026 | Low | Auth / Login form | Demo accounts (`demo.admin@llv.dev`, `demo.client@llv.dev`) not appearing in the quick-login dropdown on `/auth/login`. Root cause: dropdown was populated from a static list that predated the demo accounts. Added both to `QUICK_ACCOUNTS` in `login-form.tsx`, gated by `DEMO_LOGIN_ENABLED`. | ✅ FIXED |
| 18 | May 30, 2026 | High | Auth / Demo login | Demo login affordances (`DemoLogin` component, `signInAsDemo` action, quick-select dropdown) were gated on `process.env.NODE_ENV !== 'production'` — which is always `false` on Vercel (Next.js sets `NODE_ENV = 'production'` at build time). Demo login never worked on the deployed app. Replaced all `NODE_ENV` gates with `NEXT_PUBLIC_ENABLE_DEMO_LOGIN === 'true'` feature flag. Set the var in Vercel env to enable demo features on the staging/demo deployment without code changes. `src/actions/auth.ts`, `src/components/auth/demo-login.tsx`, `src/components/auth/login-form.tsx`. | ✅ FIXED |
