...
export default async function AdminInvestorConfigPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {configError ? (
        <AdminLoadError area="the welcome panel config" message={configError.message} />
      ) : (
        <InvestorConfigForm
          welcomeHeading={config?.welcome_heading ?? DEFAULT_WELCOME_HEADING}
          welcomeBody={config?.welcome_body ?? DEFAULT_WELCOME_BODY}
        />
      )}
    </div>
  )
